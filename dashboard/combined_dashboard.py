...
def load_all_data():
    ...
    # Ensure department exists only if missing
...
with adaptive_tab:
    ...
    with st.form("add_user_form"):
        ...
        # Display department guidance thresholds
        ...
        new_user["mean_login_hour"] = float(
            st.number_input(
                "Average Login Hour (0-23)", 
                min_value=0.0, max_value=23.0, 
                value=float((thr["mean_login_hour"][0] + thr["mean_login_hour"][1]) / 2)
            )
        )
        new_user["files_per_day"] = int(
            st.number_input(
                "Files Accessed Per Day", 
                min_value=0, max_value=10000, 
                value=int((thr["files_per_day"][0] + thr["files_per_day"][1]) // 2)
            )
        )
        new_user["usb_per_day"] = float(
            st.number_input(
                "USB Devices Per Day", 
                min_value=0.0, max_value=100.0, 
                value=float((thr["usb_per_day"][0] + thr["usb_per_day"][1]) / 2)
            )
        )
        new_user["emails_per_day"] = int(
            st.number_input(
                "Emails Sent Per Day", 
                min_value=0, max_value=10000, 
                value=int((thr["emails_per_day"][0] + thr["emails_per_day"][1]) // 2)
            )
        )
    ...
    if submitted:
        if not new_user["user"] or new_user["user"].strip() == "":
            ...
            # 🔹 Retrain the model with the new user data
            ...
            # Compute department-specific 90th percentile for high-risk
            if "isolation_forest" in updated_scores.columns:
                high_risk_threshold = updated_scores["isolation_forest"].quantile(0.90)
                user_score = updated_scores.loc[
                    updated_scores["user"] == user_id, "isolation_forest"
                ].values
                if len(user_score) > 0 and user_score[0] >= high_risk_threshold:
                    st.toast(
    ...
    try:
        ...
        if st.button("🚫 Remove Selected User"):
            ...
            if not features_df.empty:
                ...
                X = features_df.drop(columns=["user"], errors="ignore")
                iso_scores, _, _ = _train_and_save_models(X)
                ...
                scores_df["isolation_forest"] = scaler.fit_transform(scores_df[["isolation_forest"]])
